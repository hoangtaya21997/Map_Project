...
let dataMap = JSON.parse(retrievedData) || null;
...
function submitEdit(data, callback) {
    const hashId = window.location.hash.replace("#", "") || "1";
    const index = dataMap.findIndex(item => item.id === hashId);
    dataMap[index] = { ...dataMap[index], ...data };
    rootDiv.setAttribute('data', JSON.stringify(dataMap));
